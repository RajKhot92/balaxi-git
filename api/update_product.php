<?php
    ...
    if($login_id === "" || $product_id === "" || $product_name === "" || $product_category === "" || $product_owner === "" || $country_id === "" || $deadline_dt === "" || $therapeutic_id === ""){
    ...
    /*  Checking product id exist    */
    $product_id_exist = "SELECT product_id FROM product_master 
                    WHERE product_id = ".$product_id."
                    AND del_by IS NULL";
    $product_id_response = mysqli_query($conn, $product_id_exist);
    if (mysqli_num_rows($product_id_response) == 0){
    ...
    /*  Adding new product     */
    $update_product_sql = "UPDATE product_master 
    ...
    if ($conn->query($update_product_sql) !== TRUE) {
        echo "Some error occurred while updating new product. Please try again later.";
        exit();
    }
