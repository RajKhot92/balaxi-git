<?php
    ...
    if($login_id === "" || $vendor_id === "" || $vendor_name === ""){
    ...
    /*  Checking vendor name already exists     */
    $vendor_name_exist = "SELECT vendor_id FROM vendor_master 
                        WHERE vendor_id != ".$vendor_id." 
                        AND UPPER(vendor_name) = UPPER('".$vendor_name."')
                        AND del_by IS NULL";
    $vendor_name_response = mysqli_query($conn, $vendor_name_exist);
    if (mysqli_num_rows($vendor_name_response) > 0){
        echo "Entered vendor name is already exist in the system. Please try with other vendor name.";
        exit();
    }
    
    /*  Updating vendor name    */
    $update_vendor_sql = "UPDATE vendor_master 
                        SET vendor_name = UPPER('".$vendor_name."')
                        WHERE vendor_id = ".$vendor_id;

    if ($conn->query($update_vendor_sql) !== TRUE) {
        echo "Some error occurred while updating vendor. Please try again later.";
        exit();
    }
