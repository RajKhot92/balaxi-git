<?php
    ...
    if (!empty($_FILES['gmp']['name'])) {
        if ($_FILES['gmp']['error'] != 0) {
            echo 'Something wrong with the file.';
            exit();
        }

        $file_name = $_FILES['gmp']['name'];
        $file_tmp = $_FILES['gmp']['tmp_name'];
        $file_content = addslashes(file_get_contents($_FILES['gmp']['tmp_name']));
    }
    ...
    $add_gmp_sql = "INSERT INTO product_gmp (`product_id`, `validity`, `received_date`, `gmp`, `ent_by`, `ent_dt`)
                            VALUES (".$product_id.",STR_TO_DATE('".$validity."', '%m/%d/%Y'),STR_TO_DATE('".$received_date."', '%m/%d/%Y'),'".$file_content."',".$login_id.",NOW())";

    if ($conn->query($add_gmp_sql) !== TRUE) {
        echo "Some error occurred while adding gmp details. Please try again later.";
        exit();
    }
