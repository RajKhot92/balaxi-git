<?php
    ...
    $category_exist = "SELECT category_id FROM query_category 
                    WHERE category_id = ".$category_id."
                    AND del_by IS NULL";
    $category_response = mysqli_query($conn, $category_exist);
    if (mysqli_num_rows($category_response) == 0){
        echo "Query category details not found for category id ".$category_id." in the system.";
        exit();
    }

    /*  Checking category name already exists     */
    $category_name_exist = "SELECT category_id FROM query_category 
                        WHERE category_id != ".$category_id." 
    ...
    if (mysqli_num_rows($category_name_response) > 0){
        echo "Entered query category name is already exist in the system. Please try with other category name.";
        exit();
    }
    
    /*  Updating category name    */
    $update_category_sql = "UPDATE query_category 
                        SET category_name = UPPER('".$category_name."')
                        WHERE category_id = ".$category_id;

    if ($conn->query($update_category_sql) !== TRUE) {
        echo "Some error occurred while updating query category. Please try again later.";
        exit();
    }
